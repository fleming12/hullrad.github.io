...
# For saccharides use average vbar = 0.63
# Schuster, TM & Laue, TM, Eds, Modern Analytical UC, Springer, 2012
#
# V = (vbar*M)/0.60224
#          Mass         Volume  vbar      ND
GL_data = {
 'NG6':  ( 301.27,       315.2,  0.630,   0.00),
 'NAG':  ( 221.21,       231.4,  0.630,   0.00),
 'BM3':  ( 221.21,       231.4,  0.630,   0.00),
 'NGA':  ( 221.21,       231.4,  0.630,   0.00),
 'GCU':  ( 194.14,       203.1,  0.630,   0.00),
 'IDR':  ( 194.14,       203.1,  0.630,   0.00),
 'BMA':  ( 180.16,       188.4,  0.630,   0.00),
 'MAN':  ( 180.16,       188.4,  0.630,   0.00),
 'GAL':  ( 180.16,       188.4,  0.630,   0.00),
 'GLA':  ( 180.16,       188.4,  0.630,   0.00),
 'GLC':  ( 180.16,       188.4,  0.630,   0.00),
 'BGC':  ( 180.16,       188.4,  0.630,   0.00),
 'AOS':  ( 180.16,       188.4,  0.630,   0.00),
 'GCS':  ( 179.17,       187.4,  0.630,   0.00),
 'RAM':  ( 164.16,       171.7,  0.630,   0.00),
 'FUC':  ( 164.16,       171.7,  0.630,   0.00),
}
...
def model_from_pdb(file):
    ...
    gl_array = [['X' for j in range(7)] for i in range(num_atoms)]
    for row in range(len(gl_array)):
        gl_array[row][0] = row
        gl_array[row][1] = (gl_rec[row][11:16])
        gl_array[row][2] = (gl_rec[row][17:20])
        gl_array[row][3] = (gl_rec[row][22:26])
        gl_array[row][4] = (gl_rec[row][30:38])
        gl_array[row][5] = (gl_rec[row][38:46])
        gl_array[row][6] = (gl_rec[row][46:54])
...
def Sved(num_MG,num_MN,model_array):
    ...
    tot = 0.0
    for row in range(len(model_array)):
        X = X + (float(model_array[row][4]))
        Y = Y + (float(model_array[row][5]))
        Z = Z + (float(model_array[row][6]))
        tot += 1
    ...
    for row in range(len(model_array)):
        if (model_array[row][1] != '  P  '):
            Rg2 += ((distance(com_x, com_y, com_z, float(model_array[row][4]),\
                         float(model_array[row][5]), float(model_array[row][6])))**2)
    Rg = math.sqrt(Rg2/tot)

    # Calc masses for protein and nucleic acid and saccharide
    for row in range(len(model_array)):
        if (model_array[row][1] == '  CA '):
            ...
            prot_mol_mass += aa_mass
            NA = 1
        elif (model_array[row][1] == "  O5 "):
            rnam = model_array[row][2]
            aa_mass, vol_aa, vbar_aa, sc_rad_aa = GL_data[rnam]
#           print rnam, aa_mass, vol_aa, vbar_aa, sc_rad_aa
            numerator += (aa_mass * vbar_aa)
            prot_mol_mass += aa_mass
    ...
    return s,Dt,Dr,vbar_prot,Rht,ffo_hyd_P,prot_mol_mass,Ro,Rhr,int_vis,a_b_ratio,Ft,Rg,Dmax,tauC
